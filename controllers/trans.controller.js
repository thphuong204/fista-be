...
transController.createTransaction = async (req, res, next) => {
    try {
      ...
      sendResponse(
        ...
        200,
        true,
        {id, wallet, category, date, amount, description, is_deleted, created_at, updated_at},
        null,
        ""
      );
    } catch (err) {
...
transController.getTransactions = async (req, res, next) => {
    try {
      const filterKeyArr = ["wallet", "category", "fromDate", "toDate", "amount", "description"];
      ...
      const {
        wallet: tmpWallet, category: tmpCategory, 
        fromDate: fromDate, toDate: toDate, 
        amount: tmpAmount, description: tmpDescription
     } = filter;
